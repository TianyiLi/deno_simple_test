...
router
  ...
  .post("/api/refresh", async ({ response, request }) => {
    const auth = request.headers.get("authorization")?.replace(
      /^Bearer\s+/,
      "",
    );
    const jwtValidateResult = await validateJwt(auth || "", JWT_SECRET);
    if (jwtValidateResult.isValid) {
      const jwt = makeJwt({
        key: JWT_SECRET,
        header: { alg: "HS256", typ: "JWT" },
        payload: {
          iss: jwtValidateResult!.payload!.iss,
          exp: setExpiration(Date.now() + 60 * 24 * 1000),
        },
      });
      response.body = JSON.stringify({ data: jwt });
